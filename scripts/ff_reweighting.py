...
for var in plotting_variables:
  ...
  reweight_plot_variables.append(var_name)
  closure_plot_variables.append([var_name,bins_float])
...
for ind, t_ff in enumerate(total_keys):
  ...
  store[t_ff_string].AddDataframes(ConvertDatasetName(sel["Raw F_{F}"]["fail"],args.channel,args.analysis),ConvertDatasetName(sel["Raw F_{F}"]["pass"],args.channel,args.analysis),"Raw F_{F}")
  if ind + 1 != len(total_keys):
    store[t_ff_string].AddDataframes(ConvertDatasetName(sel["Alternative F_{F}"]["fail"],args.channel,args.analysis),ConvertDatasetName(sel["Alternative F_{F}"]["pass"],args.channel,args.analysis),"Alternative F_{F}")
    store[t_ff_string].AddDataframes(ConvertDatasetName(sel["Correction"]["fail"],args.channel,args.analysis),ConvertDatasetName(sel["Correction"]["pass"],args.channel,args.analysis),"Correction")
  else:
  ...
  for rwt_key in ["Raw F_{F}","Alternative F_{F}","Correction"]:
    ...
    if not args.no_plots and not args.only_mc:
      ...
      for var in closure_plot_variables:
        ...
        store[t_ff_string].PlotAverageReweights(var, rwt_key, None, data_type=None, title_left=args.channel, title_right="all_years", plot_name="reweight_ave_plot_all_{}_{}_{}_{}_all_years".format(t_ff_string,var[0],rwt_key_name,args.channel))
        ...
        if args.do_train_test_plots:
          store[t_ff_string].PlotAverageReweights(var, rwt_key, None, data_type="train", title_left=args.channel, title_right="all_years", plot_name="reweight_ave_plot_train_{}_{}_{}_{}_all_years".format(t_ff_string,var[0],rwt_key_name,args.channel))
          store[t_ff_string].PlotAverageReweights(var, rwt_key, None, data_type="test", title_left=args.channel, title_right="all_years", plot_name="reweight_ave_plot_test_{}_{}_{}_{}_all_years".format(t_ff_string,var[0],rwt_key_name,args.channel))
          ...
          store[t_ff_string].PlotClosure(var, rwt_key, None, data_type="test", title_left=args.channel, title_right="all_years", plot_name="closure_plot_test_{}_{}_{}_{}_all_years".format(t_ff_string,var[0],rwt_key_name,args.channel))

        if args.do_year_plots:
          for year in years:
            store[t_ff_string].PlotAverageReweights(var, rwt_key, "yr_{}==1".format(year), data_type=None, title_left=args.channel, title_right=year, plot_name="reweight_ave_plot_all_{}_{}_{}_{}_{}".format(t_ff_string,var[0],rwt_key_name,args.channel,year))
            ...
            if args.do_train_test_plots:
              store[t_ff_string].PlotAverageReweights(var, rwt_key, "yr_{}==1".format(year), data_type="train", title_left=args.channel, title_right=year, plot_name="reweight_ave_plot_train_{}_{}_{}_{}_{}".format(t_ff_string,var[0],rwt_key_name,args.channel,year))
              store[t_ff_string].PlotAverageReweights(var, rwt_key, "yr_{}==1".format(year), data_type="test", title_left=args.channel, title_right=year, plot_name="reweight_ave_plot_test_{}_{}_{}_{}_{}".format(t_ff_string,var[0],rwt_key_name,args.channel,year))
...
if not args.no_plots:
  ...
  proc_colours = [8,46,32,41]  

  for proc_ind, proc in enumerate(mc_procs):
    for ind, t_ff in enumerate(total_keys):
  
      t_ff_string = str(t_ff).replace("(","").replace(")","").replace(",","").replace(" ","")
      sel = MakeSelDict(t_ff,lst_n,SIDEBAND,SIGNAL,PASS,FAIL)
      store[t_ff_string].AddDataframes(ConvertMCDatasetName(sel["Signal"]["fail"],proc,args.channel,args.analysis),ConvertMCDatasetName(sel["Signal"]["pass"],proc,args.channel,args.analysis),"Signal")
    ...
    new_filename = "dataframes/{}/{}_{}_combined_signal_region_dataframe.pkl".format(args.analysis,args.channel,proc)
    ...
    for var in closure_plot_variables:
      ...
      store[t_ff_string].PlotClosure(var, "Signal", None, data_type=None, title_left=args.channel, title_right="all_years", plot_name="closure_plot_all_{}_mc_{}_{}_{}_all_years".format(proc,var[0],rwt_key_name,args.channel), fillcolour=proc_colours[proc_ind])
      if args.do_year_plots:
        for year in years:
          store[t_ff_string].PlotClosure(var, "Signal", "yr_{}==1".format(year), data_type=None, title_left=args.channel, title_right=year, plot_name="closure_plot_all_{}_mc_{}_{}_{}_{}".format(proc,var[0],rwt_key_name,args.channel,year), fillcolour=proc_colours[proc_ind])
